...
nutri_keys_para = ['total fat', 'saturated fat', 'transfat', 'cholesterol', 'sodium', 'carbohydrate',
                   ...
                   'serving size', 'calories', 'servings per container', 'trans fat']

# filename_gen = r"/Users/sakthivel/Documents/SGK/Mondelez-Word/Penang/Dataset/mondelez_word_gen_cate_model.sav"
        ...
        if classified_output == 'THAI_NUTRITION':
            ...
            if 'protein' in table_content_list[k][0].lower():
                ...
                en_nutrition.append(table_content_list[k])
                                       ...
                                       'NUTRITION_PARAGRAPH', 'THAI_NUTRITION', 'INDO_NUTRITION', 'EN_NUTRITION','ALLERGEN_STATEMENT']:
            ...
            if classified_output in ingre_dic:
                ingre_dic[classified_output].append({lang: table_content_list[k][0].replace('<b> </b>', '')})
            else:
                ingre_dic[classified_output] = [{lang: table_content_list[k][0].replace('<b> </b>', '')}]
            ...
            for d in range(1, len(final_content_list[p][0])):
                ...
                if final_content_list[p][0][d].strip() not in ['Servings per Package (EACH):',
                    ...
                    if final_content_list[p][1] in ['Legal_Designation', 'LOCATION_OF_ORIGIN', 'VARIANT',
                                                    'Servings Per Package', 'Serving Size', 'BRAND_NAME',
                ...
                if nutri_values[j1][j2].strip() != '':
                    ...
                    if nutri_keys[j2] in new_nutri_dic:
                        if '%' in nutri_values[j1][j2].strip():
                            ...
                            if 'kcal' in nutri_values[j1][j2].strip().lower() and 'energy' not in nutri_keys[j2].lower():
                                new_nutri_dic['calories'].append({'Values': {'en': nutri_values[j1][j2].strip()}})
                            else:
...
def main(file):
    ...
    over_all_dic = {**gen_cate_dic, **ingre_dic}
