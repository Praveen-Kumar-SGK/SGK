...
def griesson_processing(dictionary):
                ...
                for sentence in data.split('#'):
                    if sentence.strip():
                        ...
                        if _class in ['None','ingredients']:
                            ...
                            try:
                                lang = language_detection(sentence)
                            except:
    ...
    # print('--------' * 5)
    final_dict = lang_group_dic(final_dict,['ingredients','storage instruction','warning statement'])
    if 'Nutrition' not in final_dict:
        return {'status':'0'}
    return final_dict
        ...
        if key in spec_category:
            ...
            for list_dic in value:
                ...
                for lang, text_list in dic.items():
                    ...
                    try:
                        lang = language_detection(joined_text)
                    except:
                        lang = classify(joined_text)[0]
                    if key in final_dic:
                        final_dic[key].append({lang: joined_text})
                    else:
                        final_dic[key] = [{lang: joined_text}]
        else:
            if key in final_dic:
...
def nutrition_data_processing(input_list,method=None):
    ...
    for nutrition_row_dict in input_list:
        ...
        nutrition_header = nutrition_row_dict["1"].split('/')[0].strip()
        if method == "manual":
            nutri_probability = 1
            nutri_class = nutrition_header
        else:
            nutri_out = base('ferrero_header', ferrero_header_model).prediction(get_display(nutrition_header))
            nutri_class = nutri_out['output']
            nutri_probability = nutri_out['probability']
        ...
        if nutrition_row_dict:
            ...
            elif nutri_class not in ['header'] and len(nutrition_header) > 5:
                nutri_class = nutrition_header
                for index , value in nutrition_row_dict.items():
                    header = 'PDV' if "%" in value else 'Value'
                    value = value.strip()
